...
private class ElfMap(private val map: List<String>) {
    ...
    private fun hasNeighbor(p: Point, neighborhood: Neighborhood) =
        p.cardinalNeighbors.any { n ->
            ...
            val direction = CardinalDirection.between(ORIGIN, Point(n.x.floorDiv(size), n.y.floorDiv(size)))
...
private fun computeSubMaps(map: ElfMap, steps: Int, fullMap: Boolean): Map<Point, SubMap> {
    ...
    val activeSubMaps = mutableMapOf(ORIGIN to startMap)
...
fun main() {
    ...
    assertEquals(part1(readInput("Day21_test"), steps = 6), 16)
    assertEquals(part2(readInput("Day21_test"), steps = 6), 16)
    assertEquals(part2(readInput("Day21_test"), steps = 10), 50)
    assertEquals(part2(readInput("Day21_test"), steps = 50), 1594)
    assertEquals(part2(readInput("Day21_test"), steps = 100), 6536)
    assertEquals(part2(readInput("Day21_test"), steps = 500), 167004)
    assertEquals(part2(readInput("Day21_test"), steps = 1000), 668697)

    // ignore the slowest test
    if (false) {
        assertEquals(part2(readInput("Day21_test"), steps = 5000), 16733044)
    }

    val input = readInput("Day21")

    part1(input, steps = 64).println()
    part2(input, steps = 26501365).println()
}
